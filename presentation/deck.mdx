...
Repo including slides [here](https://github.com/icyJoseph/advent-of-code)
...
- _Day 2: 1202 Program Alarm_ 
  - **Int Code**: Learn to parse a program 📖
  - Arrive at the Moon
...
- _Day 8: Space Image Format_
  - Pixel Matrix 📸 📸 📸 to get a Password
  - Mars!
...
- _Day 21: Springdroid Adventure_ 🔥
  - **Int Code**: Run another videogame, with a scripting language 🎮
...
### 1: The Tyranny of the Rocket Equation (1)
...
<Highlighter language="rust" custom={{width:'90%', marginRight:"1em"}} >
...
### 1: The Tyranny of the Rocket Equation (2)
...
<Highlighter language="rust" fontSize="0.4em" custom={{marginRight:'1em'}}>
  {`fn calc_fuel_for_mass(mass: i32) -> i32 {
    (mass / 3) - 2
} \n 
...
renderPokemon = () => (
  this.state.currentPokemon.length > 0 && (
    this.state.currentPokemon.map((pokemon) => (
...
{`componentDidMount() {
  fetchPokemon()
    ...
    .catch(() => this.setState({ ...initialState, error: true }));
}\n
onShowMore = () => 
    this.setState(prevState => ({
